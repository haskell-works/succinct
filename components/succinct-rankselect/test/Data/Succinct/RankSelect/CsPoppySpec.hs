...
module Data.Succinct.RankSelect.CsPoppySpec (spec) where
...
import Data.Succinct.RankSelect.BasicGen
import Data.Succinct.RankSelect.CsPoppy
import Data.Succinct.RankSelect.CsPoppy.Internal.Nice
...
spec :: Spec
spec = describe "Data.Succinct.RankSelect.CsPoppySpec" $ do
  ...
  describe "Corpus specific" $
    describe "components/succinct-rankselect/data/sample-000.idx" $ do
      it "A popCount0" $ requireTest $ do
        (va :: CsPoppy) :*: (vr :: Poppy512) <- liftIO $ mmapFromForeignRegion "components/succinct-rankselect/data/sample-000.idx"
      ...
      it "Check new samples" $ requireTest $ do
        vr :: Poppy512 <- liftIO $ mmapFromForeignRegion "components/succinct-rankselect/data/sample-000.idx"

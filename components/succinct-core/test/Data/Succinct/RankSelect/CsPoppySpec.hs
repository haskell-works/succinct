...
spec = describe "Data.Succinct.RankSelect.CsPoppySpec" $ do
  ...
  describe "Corpus specific" $
    describe "components/succinct-core/data/sample-000.idx" $ do
      it "A popCount0" $ requireTest $ do
        (va :: CsPoppy) :*: (vr :: Poppy512) <- liftIO $ mmapFromForeignRegion "components/succinct-core/data/sample-000.idx"
      ...
      it "Check new samples" $ requireTest $ do
        vr :: Poppy512 <- liftIO $ mmapFromForeignRegion "components/succinct-core/data/sample-000.idx"

...
module Data.Succinct.BalancedParens.RangeMin (
    RangeMin (..),
    mkRangeMin,
) where
...
import Data.Succinct.Excess.MinExcess
import Data.Succinct.Excess.MinExcess1
...
import qualified Data.Succinct.BalancedParens.Internal.Vector.Storable as DVS
...
rm2FindClose v s p FindL0 =
    let i = p `div` 64
     in let mins = rangeMinL0Min v
         in let minE = fromIntegral (mins !!! fromIntegral i) :: Int
             in if fromIntegral s + minE <= 0
                    then rm2FindClose v s p FindBP
                    else
                        if v `newCloseAt` p && s <= 1
                            then Just p
                            else
                                let excesses = rangeMinL0Excess v
                                 in let excess = fromIntegral (excesses !!! fromIntegral i) :: Int
                                     in rm2FindClose v (fromIntegral (excess + fromIntegral s)) (p + 64) FindFromL0
rm2FindClose v s p FindL1 =
    let !i = p `div` (64 * pageSizeL1)
     in let !mins = rangeMinL1Min v
         in let !minE = fromIntegral (mins !!! fromIntegral i) :: Int
             in if fromIntegral s + minE <= 0
                    then rm2FindClose v s p FindL0
                    else
                        if 0 <= p && p < bitLength v
                            then
                                if v `newCloseAt` p && s <= 1
                                    then Just p
                                    else
                                        let excesses = rangeMinL1Excess v
                                         in let excess = fromIntegral (excesses !!! fromIntegral i) :: Int
                                             in rm2FindClose v (fromIntegral (excess + fromIntegral s)) (p + (64 * pageSizeL1)) FindFromL1
                            else Nothing
rm2FindClose v s p FindL2 =
    let !i = p `div` (64 * pageSizeL2)
     in let !mins = rangeMinL2Min v
         in let !minE = fromIntegral (mins !!! fromIntegral i) :: Int
             in if fromIntegral s + minE <= 0
                            ...
                            then
                                if v `newCloseAt` p && s <= 1
                                    then Just p
                                        ...
                                        let excesses = rangeMinL2Excess v
                                         in let excess = fromIntegral (excesses !!! fromIntegral i) :: Int
                                             in rm2FindClose v (fromIntegral (excess + fromIntegral s)) (p + (64 * pageSizeL2)) FindFromL2
                            else Nothing

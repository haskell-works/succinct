...
module Data.Succinct.BalancedParens.Internal.Broadword.FindClose.Vector8Spec
  ( spec
  ) where
...
import qualified Data.Succinct.BalancedParens.Internal.Broadword.FindClose.Vector8 as V8
...
testFiles = IO.unsafePerformIO $ do
  files <- IO.safeListDirectory "components/succinct-balancedparens/data/test"
  return $ L.sort (("components/succinct-balancedparens/data/test/" ++) <$> (".ib.idx" `L.isSuffixOf`) `filter` files)
{-# NOINLINE testFiles #-}

spec :: Spec
spec = describe "Data.Succinct.BalancedParens.Internal.Broadword.FindClose.Vector8Spec" $ do

...
makeBenchBlankJson = do
  entries <- IO.listDirectory "components/succinct-json-standard-cursor/corpus/bench"
  let files = ("components/succinct-json-standard-cursor/corpus/bench/" ++) <$> (".json" `isSuffixOf`) `filter` entries
